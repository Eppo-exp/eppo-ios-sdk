...
final class DebugCallbackTests: XCTestCase {
  ...
  func testDebugCallbackReceivesTimingData() async throws {
    ...
    stub(condition: isHost("fscdn.eppo.cloud")) { _ in
      ...
      return HTTPStubsResponse(
        data: testConfig.data(using: .utf8)!, statusCode: 200,
    ...
    if capturedMessages.count > 1 {
      ...
      XCTAssertGreaterThan(
        secondMessage.1, 0.0, "Elapsed time should be positive for subsequent messages")
      XCTAssertGreaterThan(
        secondMessage.2, 0.0, "Step duration should be positive for subsequent messages")
    }
    ...
    let isValidLastMessage =
      lastMessage.0.contains("Total SDK initialization completed")
      || lastMessage.0.contains("Starting persistent storage write")
      || lastMessage.0.contains("Persistent storage write completed")
    XCTAssertTrue(
      isValidLastMessage,
      "Last message should be SDK completion or storage write operation, but was: '\(lastMessage.0)'"
    )
  ...
  func testDebugCallbackWithPersistentStorage() async throws {
    ...
    XCTAssertGreaterThan(
      firstInitMessages.count, 0, "First initialization should generate debug messages")
    ...
    let hasReadFailure = firstInitMessageTexts.contains {
      $0.contains("failed") || $0.contains("not found")
    }
    let hasNetworkFetch = firstInitMessageTexts.contains {
      $0.contains("fetch") || $0.contains("network")
    }
    let hasStorageWrite = firstInitMessageTexts.contains { $0.contains("persistent storage write") }

    XCTAssertTrue(
      hasReadAttempt || hasReadFailure,
      "First initialization should attempt to read from persistent storage and fail")
    ...
    let hasNoStorageOperations = !secondInitMessageTexts.contains {
      $0.contains("persistent storage")
    }
    XCTAssertTrue(
      hasNoStorageOperations || secondInitMessages.isEmpty,
      "Second initialization should not perform storage operations")
    ...
    if !secondInitMessages.isEmpty {
      ...
      XCTAssertLessThan(
        secondInitTotalTime, firstInitTotalTime * 0.5,
        "Second initialization should be faster (reusing existing instance)")
    }

    // Assert third initialization behavior
    XCTAssertGreaterThan(
      thirdInitMessages.count, 0, "Third initialization should generate debug messages")

    let thirdInitMessageTexts = thirdInitMessages.map { $0.0 }
    let hasSuccessfulRead = thirdInitMessageTexts.contains {
      $0.contains("persistent storage read") && !$0.contains("failed") && !$0.contains("not found")
    }

    XCTAssertTrue(
      hasSuccessfulRead, "Third initialization should successfully read from persistent storage")
  ...
  func testDebugCallbackIncludesDataSizes() async throws {
    ...
    stub(condition: isHost("fscdn.eppo.cloud")) { _ in
      return HTTPStubsResponse(
        data: testConfigData.data(using: .utf8)!, statusCode: 200,
        headers: ["Content-Type": "application/json"])
    }
    ...
    let jsonParsingMessages = messageTexts.filter {
      $0.contains("JSON parsing") && $0.contains("bytes")
    }
    XCTAssertGreaterThan(
      jsonParsingMessages.count, 0, "Should have JSON parsing message with byte count")

    // Verify the JSON parsing message contains the expected data size
    let actualJsonSizeFromMessage =
      jsonParsingMessages.first?.components(separatedBy: " ").first(where: { Int($0) != nil })
      ?? "unknown"
    let hasCorrectSize = jsonParsingMessages.contains { $0.contains("\(expectedJsonSize) bytes") }
    XCTAssertTrue(
      hasCorrectSize,
      "JSON parsing message should contain correct byte count (\(expectedJsonSize) bytes). Actual JSON size: \(actualJsonSizeFromMessage) bytes"
    )

    // Verify persistent storage messages include byte counts
    let storageWriteMessages = messageTexts.filter {
      ($0.contains("Encoded JSON configuration data")
        || $0.contains("Encoded protobuf configuration data")) && $0.contains("bytes")
    }
    XCTAssertGreaterThan(
      storageWriteMessages.count, 0, "Should have storage write message with byte count")
    ...
    let actualEncodedSizeFromMessage =
      storageWriteMessages.first?.components(separatedBy: " ").first(where: { Int($0) != nil })
      ?? "0"
    ...
    XCTAssertTrue(
      hasCompressedEncodedSize,
      "Storage write message should contain compressed encoded byte count (0 < \(actualEncodedSize) < \(expectedJsonSize))"
    )
  }
  ...
  func testDebugCallbackWithPersistentCacheDisabled() async throws {
    ...
    XCTAssertFalse(
      hasStorageRead, "Should not attempt to read from persistent storage when disabled")
    XCTAssertFalse(
      hasStorageWrite, "Should not attempt to write to persistent storage when disabled")
    ...
    XCTAssertGreaterThan(
      jsonParsingMessages.count, 0, "Should still have JSON parsing message with byte count")

    // Verify network operations still occur
    let hasNetworkOperation = messageTexts.contains {
      $0.contains("network") || $0.contains("fetch")
    }
