...
public class EppoClient {
  ...
  public static func initialize(
    ...
    pollingJitterMs: Int = PollerConstants.DEFAULT_POLL_INTERVAL_MS
    ...
    guard isValueOfType(expectedType: expectedVariationType, variationValue: variation.value) else {
      return (
        ...
        flagEvaluationDescription:
          "Variation (\(variation.key)) is configured for type \(expectedVariationType), but is set to incompatible value (\(variation.value))"
      )
    }
  ...
  public func getStringAssignmentDetails(
    ...
    do {
      ...
      let evaluationDetails = FlagEvaluationDetails(
        environmentName: configurationStore.getConfiguration()?.getFlagConfigDetails()
        ...
        banditKey: nil,
        banditAction: nil,
        configFetchedAt: configurationStore.getConfiguration()?.getFlagConfigDetails()
          .configFetchedAt ?? "",
        configPublishedAt: configurationStore.getConfiguration()?.getFlagConfigDetails()
          .configPublishedAt ?? "",
  ...
  public func getJSONStringAssignmentDetails(
    ...
    do {
      ...
      let finalVariation =
        (variation == nil && flagEvaluation?.flagEvaluationCode == .flagUnrecognizedOrDisabled)
        ? nil : (variation ?? defaultValue)
  ...
  public func startPolling(
    ...
    jitterMs: Int = PollerConstants.DEFAULT_POLL_INTERVAL_MS
      / PollerConstants.DEFAULT_JITTER_INTERVAL_RATIO
  ) async throws {
  ...
  private func debugLog(_ message: String) {
    ...
    let stepDurationMs =
      debugLastStepTime != nil ? now.timeIntervalSince(debugLastStepTime!) * 1000.0 : 0.0
