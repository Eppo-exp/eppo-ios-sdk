...
public struct Configuration {
  ...
  internal init(
    flagsConfiguration: UniversalFlagConfig?, flagsConfigurationProtobuf: Ufc_UniversalFlagConfig?,
    obfuscated: Bool, fetchedAt: String, publishedAt: String
  ) {
  ...
  public init(flagsConfigurationProtobuf: Data, obfuscated: Bool) throws {
    let flagsConfiguration = try Ufc_UniversalFlagConfig(
      serializedBytes: flagsConfigurationProtobuf)
    let formatter = ISO8601DateFormatter()
    formatter.formatOptions = [.withInternetDateTime, .withFractionalSeconds]
    let now = formatter.string(from: Date())
    let createdAtDate = Date(
      timeIntervalSince1970: TimeInterval(flagsConfiguration.createdAtMs) / 1000)
    self.init(
      ...
      if self.flagsPb != nil {
        throw EncodingError.invalidValue(
          ...
          EncodingError.Context(
            codingPath: [],
            debugDescription:
              "Cannot convert protobuf configuration to JSON - this operation is not supported to avoid wasteful conversion"
          ))
      } else {
        throw EncodingError.invalidValue(
          self,
      ...
      if self.flagsConfiguration != nil {
        throw EncodingError.invalidValue(
          ...
          EncodingError.Context(
            codingPath: [],
            debugDescription:
              "Cannot convert JSON configuration to protobuf - this operation is not supported to avoid wasteful conversion"
          ))
      } else {
        throw EncodingError.invalidValue(
          self,
