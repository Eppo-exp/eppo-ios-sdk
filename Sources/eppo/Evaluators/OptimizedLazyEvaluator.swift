...
public class OptimizedLazyEvaluator {
    ...
    func evaluateFlag(
        ...
        // Get or load the flag from cache using optimized access
        guard let ufcFlag = getOrLoadFlagOptimized(flagKey: flagKey) else {
    ...
    // OPTIMIZED: Fast cache access with minimal locking
    private func getOrLoadFlagOptimized(flagKey: String) -> UFC_Flag? {
        // Fast path: Try to read from cache without locking first
        // This works because dictionary reads are atomic in Swift for reference types
        if let cachedFlag = flagCache[flagKey] {
            return cachedFlag
        }

        // Slow path: Need to load and cache the flag
        cacheLock.lock()
        defer { cacheLock.unlock() }

        // Double-check pattern: flag might have been loaded by another thread
        if let cachedFlag = flagCache[flagKey] {
            return cachedFlag
        }
