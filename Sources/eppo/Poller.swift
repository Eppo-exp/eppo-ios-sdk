...
class RealTimer: TimerType {
    ...
    func schedule(deadline: TimeInterval, callback: @escaping () async -> Void)
    {
        ...
        timer = Timer.scheduledTimer(withTimeInterval: deadline, repeats: false)
        { _ in
            Task {
                await callback()
            }
...
// Mock timer implementation for fast testing
@MainActor
class MockTimer: TimerType {
    private(set) var executeCount: Int = 0
    private var isRunning = true
    private var scheduledCallbacks:
        [(deadline: TimeInterval, callback: () async -> Void)] = []
    ...
    func schedule(deadline: TimeInterval, callback: @escaping () async -> Void)
    {
        ...
        if isRunning {
            // Store the callback and deadline for manual execution
            scheduledCallbacks.append((deadline: deadline, callback: callback))
        }
    }
    ...
    // Execute the next scheduled callback immediately
    func executeNext() async {
        guard isRunning && !scheduledCallbacks.isEmpty else { return }
        let next = scheduledCallbacks.removeFirst()
        await next.callback()
    }
    ...
    // Execute all scheduled callbacks immediately
    func executeAll() async {
        while isRunning && !scheduledCallbacks.isEmpty {
            await executeNext()
        }
    }
    ...
    var hasPendingCallbacks: Bool {
        return !scheduledCallbacks.isEmpty
    }

    // Get the number of pending callbacks
    var pendingCallbackCount: Int {
        return scheduledCallbacks.count
    }
}
...
public class Poller {
    ...
    private func schedulePoll() {
        ...
        timer.schedule(deadline: TimeInterval(nextInterval) / 1000.0) {
            [self] in
            await poll()
        }
    }
    ...
    private func poll() async {
        ...
        do {
            ...
            logger.warn(
                "Eppo SDK encountered an error polling configurations: \(error.localizedDescription)"
            )
            failedAttempts += 1
            if failedAttempts < PollerConstants.DEFAULT_MAX_POLL_RETRIES {
                ...
                let remainingAttempts =
                    PollerConstants.DEFAULT_MAX_POLL_RETRIES - failedAttempts
                logger.warn(
                    "Eppo SDK will try polling again in \(nextPollMs) ms (\(remainingAttempts) attempts remaining)"
                )
            } else {
                logger.error(
