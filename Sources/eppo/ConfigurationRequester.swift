...
class ConfigurationRequester {
  ...
  public func fetchConfigurations() async throws -> Configuration {
    ...
    if let httpResponse = response as? HTTPURLResponse {
      debugLogger?(
        "Network request completed (\(String(format: "%.1f", networkDurationMs))ms, Status: \(httpResponse.statusCode), Data: \(data.count) bytes)"
      )
    } else {
      debugLogger?(
        "Network request completed (\(String(format: "%.1f", networkDurationMs))ms, Data: \(data.count) bytes)"
      )
    }
    ...
    if requestProtobuf {
      debugLogger?(
        "Starting protobuf parsing and configuration creation (parsing \(data.count) bytes)")
